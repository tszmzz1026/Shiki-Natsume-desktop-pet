...
def test_tts_request_payload_keeps_japanese_text_ordered(monkeypatch) -> None:  # type: ignore[no-untyped-def]
    """GPT-SoVITS 请求应保持日文整句并按顺序串行合成。"""
    ...
    monkeypatch.setattr(provider, "_audio_ready", types.SimpleNamespace(emit=lambda *_args: None))
    monkeypatch.setattr(provider, "_prepared_audio_ready", types.SimpleNamespace(emit=lambda *_args: None))
    ...
    provider._request_audio(request)  # type: ignore[arg-type]
...
def test_tts_provider_restarts_stale_adopted_service(monkeypatch) -> None:  # type: ignore[no-untyped-def]
    """接管已有服务时，健康探测失败应清理旧进程并解除接管。"""
    work_dir = _runtime_root("gptsovits_stale_restart") / "gpt-sovits"
    ...
    provider = GPTSoVITSTTSProvider(
        _minimal_tts_settings(work_dir=work_dir),
    ...
    provider._server_process = FakeProcess()  # type: ignore[assignment]

    def fake_urlopen(*_args: object, **_kwargs: object) -> object:
        raise urllib.error.HTTPError("http://127.0.0.1:9880/tts", 400, "bad", {}, None)

    monkeypatch.setattr("app.voice.tts.urllib.request.urlopen", fake_urlopen)
    monkeypatch.setattr(
        "app.voice.tts._terminate_process_tree",
        lambda process, timeout: killed.append(process),
    )

    assert provider._adopted_service_is_healthy() is False
    provider._restart_stale_local_service()

    assert len(killed) == 1
    assert provider._server_process is None


def test_tts_provider_skips_health_probe_without_adopted_process() -> None:
    """未接管本地进程或外部服务时不执行合成探测，避免误杀。"""
    provider = types.SimpleNamespace(_server_process=None)

    assert GPTSoVITSTTSProvider._adopted_service_is_healthy(provider)

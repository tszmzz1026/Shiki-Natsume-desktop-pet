...
- `data/config/api.yaml` 含 API Key，永远不要提交到公开仓库。
- 不要把聊天历史、记忆、屏幕截图数据复制到 GitHub。
- `runtime/`、`tts/`、`temp/` 保持不变，不加入版本控制。
- 高风险工具默认需要确认；不要在不可信的模型服务中使用“完整访问权限”。
- 角色语音模型与立绘属于本地/可分发资源，确认版权后再公开传播。

...
# 结束前允许的播放时长误差（毫秒）
_SINK_DRAIN_GUARD_MS = 100
# 排空重试延时范围（毫秒）
_SINK_DRAIN_RETRY_MIN_MS = 50
_SINK_DRAIN_RETRY_MAX_MS = 500
...
class AudioSinkPlayer(QObject):
    ...
    def _finish_if_drained(self) -> None:
        ...
        retry_delay_ms = _drain_retry_delay_ms(elapsed_ms, self._duration_ms)
        if retry_delay_ms is None and elapsed_ms >= self._duration_ms + _SINK_DRAIN_MAX_MS:
            # Hard cap: never wait forever when state/buffer checks are unreliable
            self._finish_once("drain_after_all_pcm_written")
            return

        if retry_delay_ms is not None or _sink_still_buffering(self._sink):
            wait_ms = retry_delay_ms if retry_delay_ms is not None else _SINK_DRAIN_RETRY_MIN_MS
            debug_log(
                "TTS",
                "AudioSink: 实际播放时长不足，继续等待排空",
                {
                    "audio_path": str(self._audio_path) if self._audio_path else "",
            ...
            QTimer.singleShot(wait_ms, self._finish_if_drained)
            return

        # Playback duration elapsed and sink buffer drained, safe to finish
        self._finish_once("drain_after_all_pcm_written")
...
def _drain_retry_delay_ms(elapsed_ms: int, duration_ms: int) -> int | None:
    """返回距音频自然结束还需等待的毫秒；不足守护窗口时返回 None 表示可结束。"""
    if duration_ms <= 0:
        return None
    remaining_ms = duration_ms - elapsed_ms
    if remaining_ms <= _SINK_DRAIN_GUARD_MS:
        return None
    delay_ms = remaining_ms - _SINK_DRAIN_GUARD_MS
    return max(_SINK_DRAIN_RETRY_MIN_MS, min(_SINK_DRAIN_RETRY_MAX_MS, delay_ms))


def _sink_still_buffering(sink_like: object) -> bool:
    """Whether QAudioSink still has buffered audio; conservative when unknown."""
    if sink_like is None:
        return False
    try:
        state = sink_like.state()

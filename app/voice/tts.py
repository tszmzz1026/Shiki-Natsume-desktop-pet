...
class GPTSoVITSTTSProvider(QObject):
    ...
    def _ensure_service_available(
        ...
        if GPTSoVITSTTSProvider._probe_service_port(self, host, port, timeout, purpose=probe_purpose):
            ...
            if GPTSoVITSTTSProvider._adopted_service_is_healthy(self):
                self._service_checked = True
                debug_log("TTS", "服务探测成功", {"api_url": self.settings.api_url})
                return True
            GPTSoVITSTTSProvider._restart_stale_local_service(self)
            debug_log(
                "TTS",
                "接管的本地 TTS 服务异常，已清理，准备重新启动",
                {"api_url": self.settings.api_url},
            )
    ...
    def _adopted_service_is_healthy(self) -> bool:
        """接管已有本地服务时做一次最小合成探测，识别上一实例遗留的坏服务。"""
        process = getattr(self, "_server_process", None)
        if process is None:
            return True
        settings = getattr(self, "settings", None)
        select_reference = getattr(self, "_select_reference", None)
        if settings is None or settings.work_dir is None or not callable(select_reference):
            return True
        try:
            reference = select_reference(None)
            text = _tts_probe_text(settings.text_lang)
            payload = {
                "text": text,
                "text_lang": _resolve_request_text_lang(text, settings.text_lang),
                "ref_audio_path": str(reference.ref_audio_path),
                "prompt_text": reference.ref_text,
                "prompt_lang": reference.ref_lang,
                "text_split_method": "cut1",
            ...
            with urllib.request.urlopen(
                request,
                timeout=min(settings.timeout_seconds, 15),
            ) as response:
                audio_data = response.read()
            if not audio_data:
                raise ValueError("empty probe audio")
        except Exception as exc:  # noqa: BLE001
            debug_log("TTS", "接管的本地 TTS 服务健康探测失败", {"error": str(exc)})
            return False
        return True

    def _restart_stale_local_service(self) -> None:
        """杀掉上一实例遗留的坏 TTS 服务，空出端口交给新的本地服务。"""
        process = getattr(self, "_server_process", None)
        ...
        try:
            ...
            debug_log("TTS", "清理遗留 TTS 服务失败，将尝试直接启动新服务", {"error": str(exc)})
...
def _tts_probe_text(text_lang: str) -> str:
    """返回健康探测用的一句话，尽量贴合目标语音语言。"""
    normalized = text_lang.strip().lower()
    if normalized in {"zh", "all_zh"}:
        return "你好。"
    if normalized in {"en", "all_en"}:
        return "Hello."
    return "こんにちは。"
